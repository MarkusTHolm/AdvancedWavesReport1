...
\begin{axis}[
	...
	ylabel = Normalized net mass flow: \\ $\dot{m}_{\text{net}}/\dot{m}_{\text{in}}$ \si{[\percent]} ,
	minor tick num = 0,
    ...
    at={(1,-0.2)},xshift=0pt,anchor=south west,inner sep=0pt}
]
\addplot table [black, solid, mark = *,mark options={fill=white, scale=0.7}]{Figures/Plotdata/JONSWAPres.dat};
\legend{
        $\dot{m}_{\text{net}}/\dot{m}_{\text{in}}(h^2)$}
\end{axis}
\end{tikzpicture}
\caption{ }
